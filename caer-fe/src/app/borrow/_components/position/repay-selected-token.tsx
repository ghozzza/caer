...
export const RepaySelectedToken = ({
  lpAddress,
  borrowToken,
}: {
  lpAddress: string;
  ...
  const {
    ...
    error: repayError,
    reset: resetRepay,
  } = useRepayWithSelectedToken(borrowToken, lpAddress, true);
  ...
  useEffect(() => {
    if (isRepayPending || isRepayLoading) {
  ...
  }, [isRepayPending, isRepayLoading, isRepaySuccess]);
        ...
        <div className="space-y-6 py-4">
            ...
            <Card className="border border-red-200 bg-red-50 shadow-sm">
              <CardContent className="p-3">
                <div className="flex items-center gap-2 text-sm text-red-600">
                  <span>⚠️</span>
                  <span>{repayError?.message || "Transaction failed"}</span>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {repayHash && !isRepaySuccess && (
            <div>
              ...
              >
                {repayHash?.slice(0, 20)}...{repayHash?.slice(-10)}
                <ExternalLink className="h-3 w-3" />
              </a>
            </div>
          )}
